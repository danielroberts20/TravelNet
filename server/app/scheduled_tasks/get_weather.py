...
from config.general import COORD_PRECISION, DAILY_VARS, HOURLY_VARS, OPEN_METEO_FORECAST_URL, OPEN_METEO_URL, REQUEST_DELAY, WEATHER_LAG_DAYS, WEATHER_LOOKBACK_DAYS
...
def _fetch_uv(lat: float, lon: float, start_date: date, end_date: date, logger) -> dict | None:
    ...
    try:
        ...
        logger.warning("UV fetch failed for (%.2f, %.2f): %s", lat, lon, exc)
        return None
...
def fetch_and_store_all_weather(
    ...
    for lat, lon in locations:
        ...
        # UV index (forecast endpoint)
        uv_data = _fetch_uv(lat, lon, start_date, end_date, logger)
        if uv_data is None:
            uv_failed += 1
        elif hourly_data is not None:
            uv_values = uv_data.get("hourly", {}).get("uv_index", [])
            hourly_data["hourly"]["uv_index"] = uv_values
        time.sleep(REQUEST_DELAY)
    ...
    if hourly_failed or daily_failed or uv_failed:
        logger.warning(
            "%d hourly, %d UV, and %d daily cell(s) failed to fetch from Open-Meteo.",
