...
def test_revolut_missing_auth_rejected(revolut_client):
    c, _ = revolut_client
    csv_content = make_revolut_csv([])
    resp = c.post(
        "/upload/transaction/revolut",
...
def test_revolut_insert_called_with_saved_path(revolut_client, tmp_path):
    ...
    called_path = mock_insert.call_args[0][0]
    assert str(called_path).endswith(".csv")
...
@pytest.fixture
def wise_client(db):
    with patch("upload.transaction.endpoints.insert_wise"), \
         patch("upload.transaction.endpoints.check_auth") as mock_insert:
...
def test_wise_missing_auth_rejected(wise_client):
    c, _ = wise_client
    resp = c.post(
        "/upload/transaction/wise",
        files={"file": ("export.zip", _make_zip(), "application/zip")},
